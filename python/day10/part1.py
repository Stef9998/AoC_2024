...
def score_of_map(trail_map: list[list[int]]):
    ...
    return sum(len(score_of_trail(coordinate, 0, trail_map, set())) for coordinate in start_coordinates)


def score_of_trail(coordinate: Coordinate, expected_number: int, trail_map: list[list[int]],
                   found_ends: set[Coordinate]):
    """
    ...
    Args:
        ...
        found_ends (set[tuple[int, int]]): The set of coordinates that the trail should ends.

    Returns:
        set[tuple[int, int]]: The found ends
    ...
    Examples:
        ...
        ...     [ 9,-1,-1,-1,-1,-1, 9],
        ... ]
        >>> score_of_trail(Coordinate(0, 3), 0, trail_map_, set())
        {Coordinate(6, 6), Coordinate(6, 0)}
    """
    y = coordinate[0]
    x = coordinate[1]
    ...
    # )
    for neighbor_coordinate in get_surrounding_coordinates(coordinate):
        found_ends = score_of_trail(neighbor_coordinate, number + 1, trail_map, found_ends)
    return found_ends
