...
@lru_cache()
def get_shortest_path(pad_type: PadType, start, end):
    def rec(current, visitied: set):
        if current is end:
            return []
        new_visited = visitied | {current}
        options = pad_type.value[current]
        next_keys = set(options.keys()).difference(visitied)
        if len(next_keys) == 0:
            return None

        res_res = None
        next_length = sys.maxsize
        for key in next_keys:
            direction = options[key]
            res = rec(key, new_visited)
            if res is None:
                continue
            if len(res) < next_length:
                next_length = len(res)
                res_res = [direction] + res
        return res_res
...
if __name__ == "__main__":
    ...
    for _ in range(iterations):
        for start_key in keys:
            for end_key in keys:
                get_shortest_path(PadType.NumPad, start_key, end_key)
    t1 = time.time()
